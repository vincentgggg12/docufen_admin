...
test('TS.7.6.2-01 Owner Only Reversion', async ({ page, context }) => {
  ...
  // Create a document and advance it to a later stage
  ...
  // Advance document to Execution stage if possible
  const forwardButton = page.getByRole('button', { name: /Next Stage|Forward|Advance|To Execution/i });
  ...
  await test.step('Back button visible.', async () => {
    // Look for back/revert stage buttons
  ...
  // Step 4: No back button
  await test.step('No back button.', async () => {
    const johannaPage = page.johannaPage;
    
    // Verify back/revert buttons are not visible for participant
    const backButton = johannaPage.getByRole('button', { name: /Back|Previous|Revert|Previous Stage/i });
  ...
  await test.step('Owner restricted (SC)', async () => {
    ...
    // Take screenshot showing absence of back button for participant
    const timestamp = new Date();
    const formattedTimestamp = `${timestamp.getFullYear()}.${String(timestamp.getMonth() + 1).padStart(2, '0')}.${String(timestamp.getDate()).padStart(2, '0')}-${String(timestamp.getHours()).padStart(2, '0')}.${String(timestamp.getMinutes()).padStart(2, '0')}.${String(timestamp.getSeconds()).padStart(2, '0')}`;
    await johannaPage.screenshot({ 
      path: getScreenshotPath(`TS.7.6.2-01-5-${formattedTimestamp}.png`) 
    });
